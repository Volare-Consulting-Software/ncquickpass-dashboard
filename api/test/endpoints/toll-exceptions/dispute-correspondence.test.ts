...
const updated = (
  ...
  emailText:
    ...
    `<p><b>Case Status:</b> ${status}</p><p><b>Case Type:</b> Account Dispute</p>` +
    `<p><b>Case Topic:</b> Toll Dispute</p>` +
    `<p><b>Case Last Updated:</b> 10/15/2024 9:56:20 AM</p>` +
...
describe('parseDisputes', () => {
  it('parseDisputes_createdConfirmationOnly_isFiledUnderReview', () => {
    ...
    expect(dispute.decisionNotes).toBeNull();
  ...
  it('parseDisputes_caseUpdatedClosedWithDenial_isDenied', () => {
    ...
    expect(dispute.decisionNotes).toContain('deny your dispute');
    expect(dispute.decisionNotes).not.toContain('Thank you for your business');
    expect(dispute.lastUpdated).toBe('10/15/2024 9:56:20 AM');
  });
  ...
  it('parseDisputes_multipleUpdates_usesMostRecentStatus', () => {
    const rows = [
